...
assembler_err NativeTranslator(asm_t* assembler, char* assembler_text, size_t* asm_index)
{
    ...
    {
        assembler->native_code[*asm_index] = OP_POP;
        return NoErr;
    }
    else if (strcmp(assembler_text, "DUMP") == 0)
    {
        assembler->native_code[*asm_index] = OP_DUMP;
        return NoErr;
    }
    else if (strcmp(assembler_text, "MUL") == 0)
    {
        assembler->native_code[*asm_index] = OP_MUL;
        return NoErr;
    }
    else if (strcmp(assembler_text, "SUB") == 0)
    {
        assembler->native_code[*asm_index] = OP_SUB;
        return NoErr;
    }
    else if (strcmp(assembler_text, "OUT") == 0)
    {
        assembler->native_code[*asm_index] = OP_OUT;
        return NoErr;
    }
    else if (strcmp(assembler_text, "DIV") == 0)
    {
        assembler->native_code[*asm_index] = OP_DIV;
        return NoErr;
    }
    else if (strcmp(assembler_text, "HLT") == 0)
    {
        assembler->native_code[*asm_index] = OP_HLT;
        return NoErr;
    }
    else if (ComparePush(assembler_text, assembler, asm_index))
    {
        return NoErr; 
    }
    return ReadError;
...
bool ComparePush(char* assembler_text, asm_t* assembler, size_t* asm_index)
{
    if(assembler_text[0] == 'P' && assembler_text[1] == 'U' && assembler_text[2] == 'S' && assembler_text[3] == 'H')
    {
        ...
        assembler->native_code[*asm_index++] = value;
        return true;
    }
    return false;
}